...
**Unified claim contract, two mechanisms.** Both stores honor the same rule — a key bound to a `FAILED` attempt is not bound to that attempt's request, so a retry (even with a different payload) starts a fresh attempt rather than 409ing; a key bound to `COMPLETED` stays bound until it expires (see Retention below), so a same-payload retry replays and a different-payload retry conflicts. PostgreSQL gets this for free: the claim insert, the business mutation, and the completion write share one JPA transaction (`IdempotencyClaimRunner`), so a failed business operation rolls the claim back to non-existent instead of leaving a `FAILED` row — there is nothing to reclaim. MongoDB has no shared transaction manager with the business write, so `VaultOperationClaimCoordinator` emulates the same outcome explicitly: a failed attempt is written as a `FAILED` document, and a retry reclaims it via an atomic compare-and-set `findAndModify` (`FAILED` → `PROCESSING`, conditioned on `state == FAILED` so only one concurrent retry wins the race).
...
- **Adding an entry**: every improvement that changes what users see or how they interact with the UI must bump a new `version` in `changelog-entries.ts` and add its `titleKey`/`bodyKey` under `changelog.entries.<version>` in all 4 message files, in the same PR as the feature it announces.
- **Coverage gate**: the `Changelog Coverage` CI job compares a pull request's base and head revisions. Changes under rendered app routes, shared components, locale messages, or frontend styling/configuration require a higher changelog version with complete translations. Run it locally from `frontend` with `npm run check:changelog-coverage -- --base <base-revision> --head <head-revision>`.
- **Exemptions**: a UI-bearing file change that has no visible or interactive effect may use the `skip-whats-new` pull-request label with a written reason. It is not valid for a user-visible improvement; reviewers must require an entry instead.
- **Branch protection**: configure `Changelog Coverage` as a required status check for `main` after the workflow is merged, so a missing announcement cannot be merged.
