...
Every `create_*` tool requires the caller to supply an `idempotencyKey` argument (and `create_transactions_batch` additionally requires a `clientRequestId` on every row) — it is forwarded to the backend as the `Idempotency-Key` header, never generated by this server. `update_*` tools remain PUT-based and do not require one. If a call throws `FintrackApiError`, check `error.outcome`: `"definitive"` means the backend received and rejected the request outright (bad input, auth, not found, or an idempotency-key payload conflict) — retrying needs a new key or corrected input, not a blind retry. `"unknown"` means no confirmed response reached the client (network/timeout) or the backend may have partially committed before failing (5xx) — the caller must retry the exact same operation with the exact same `idempotencyKey` (or inspect current state first), and must never generate a new key for an unknown-outcome retry, since that would defeat the replay protection the key exists for.
